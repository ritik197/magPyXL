...
def _mixed_arg(data: Any):
    ...
    and gets a zero-copy view), otherwise a plain list (via `_to_list`).

    Important subtlety: `PyReadonlyArray1` extraction only succeeds on
    an actual `numpy.ndarray` instance — a pandas/polars Series merely
    *wraps* one, so handing Rust the Series object directly makes the
    numpy extraction fail and silently fall back to the slow generic
    Vec<PyObject> path (confirmed by direct benchmark: ~150ms instead of
    ~2ms for 1,000,000 rows). Pulling the underlying array out here with
    `.to_numpy()` is what actually makes the fast path engage.
    ...
    module = type(data).__module__
    if module == "numpy":
        return data  # already a raw ndarray — Rust takes float64 or int64 natively, zero-copy either way
    if module.startswith("pandas"):
        if data.dtype == "int64":
            return data.to_numpy(copy=False)  # zero-copy, no cast
        return data.to_numpy(dtype="float64", copy=False)
    if module.startswith("polars"):
        arr = data.to_numpy()
        if arr.dtype == "int64":
            return arr  # zero-copy, no cast
...
def COUNTIF(range_: Iterable, criteria: Any):
    ...
    if _is_numeric_array_backed(range_):
...
def COUNTIFS(*args):
    ...
    # All-scalar criteria. The mixed path is only a genuine win when at
    # least one range is numeric-array-backed (confirmed by direct
    # benchmark: a pure all-text COUNTIFS is slightly SLOWER through
    # FastColumn's per-row enum dispatch than the original generic path;
    # a mix of one text + one numeric range is faster, since the numeric
    # column still gets true zero-copy access). So: any numeric range
    # present -> mixed; all-text -> the original, already-correct path.
    if any(_is_numeric_array_backed(r) for r in ranges):
